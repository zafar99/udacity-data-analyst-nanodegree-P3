...
def update_name(street_name, mapping):
    for abbr, full in mapping.iteritems():
        if ' %s.' % abbr in street_name:
            return street_name.replace(' %s.' % abbr, ' %s' % full)
        elif ' %s' % abbr in street_name:
            return street_name.replace(' %s' % abbr, ' %s' % full)
...
"phone": "1 (773)-271-5176"
 ...
 <node id="261114295" visible="true" version="7" changeset="11129782" timestamp="2012-03-28T18:31:23Z" user="bbmiller" uid="451048" lat="41.9730791" lon="-87.6866303"/>
        ...
        for child in el:
            ...
            elif 'k' in child.attrib and child.attrib['k'] == 'addr:amenity':
                node['amenity'] = child.attrib['v']
            elif 'k' in child.attrib and child.attrib['k'] == 'addr:cuisine':
                node['cuisine'] = child.attrib['v']
            elif 'k' in child.attrib and child.attrib['k'] == 'addr:name':
                node['name'] = child.attrib['v']
            elif 'k' in child.attrib and child.attrib['k'] == 'addr:phone':
